<?php
/*
El parámetro de $extension determina que tipo de imagen no se borrara por ejemplo si es jpg significa que la imagen con extension .jpg se queda en el
servidor y si existen imagenes con el mismo nombre pero con extension png o gif se eliminaran, con esta función evito tener imagenes duplicadas con 
distintas extensiones para cada perfil la función file_exists evalúa si un archivo existe y la función unlink borra un archivo del servidor
*/
...
function subir_imagen($tipo,$imagen,$email) {
  /*
  strstr($cadena1,$cadena2) sirve para evaluar sin en la primera cadena de texto existe la segunda cadena de texto
  Si dentro del tipo del archivo encuentra la palabra imagen significa que el archivo es una imagen
  */
  if (strstr($tipo,"image")) {
    ...
    if ($ancho_img > $ancho_img_deseado) {
      ...
      //guardo la ruta que tendrá en el servidor la imagen
      $destino = "../img/fotos/".$email.$extension;

      //Se sube la foto 
      move_uploaded_file($imagen, $destino) or die ("No se pudo subir la imagen al servidor :c");
